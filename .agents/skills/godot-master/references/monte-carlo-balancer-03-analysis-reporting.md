...
Global `--json` → serde pretty aggregates. Stable field names per [json-schema.md](monte-carlo-balancer-json-schema.md). Include `input_model` on every cell aggregate. Always include `seed`, `runs`, `game_data_hash`.
...
Related skills:
- resource-data-patterns.md — Resource-first extract
- testing-patterns.md — Phase 7 headless calibration
Parent skill: monte-carlo-balancer.md
-->
