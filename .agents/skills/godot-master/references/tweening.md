...
| Menu motion while `Engine.time_scale == 0` | Tween with `set_ignore_time_scale(true)` — **MANDATORY** [time_scale_ignored_ui.gd](../scripts/tweening_time_scale_ignored_ui.gd) |
| Retriggerable property (button spam, dodge cancel) | Kill-before-recreate — **MANDATORY** [safe_tween_interruption.gd](../scripts/tweening_safe_tween_interruption.gd) |
...
### [juice_manager.gd](../scripts/tweening_juice_manager.gd) / [tween_builder.gd](../scripts/tweening_tween_builder.gd)
...
**MANDATORY pattern source:** [safe_tween_interruption.gd](../scripts/tweening_safe_tween_interruption.gd).
...
Parallel block → `chain()` → interval/callback → exit. Prefer [nested_subtween_cutscene.gd](../scripts/tweening_nested_subtween_cutscene.gd) for nested modules.
