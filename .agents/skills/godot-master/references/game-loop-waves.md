...
| :--- | :--- | :--- |
| **Under ~80 SceneTree enemies** | Node manager + Marker spawners | [wave_manager.gd](../scripts/game_loop_waves_wave_manager.gd), [wave_spawner.gd](../scripts/game_loop_waves_wave_spawner.gd), [wave_resource.gd](../scripts/game_loop_waves_wave_resource.gd) |
| **Swarm visuals / hundreds** | MultiMesh + weighted composition | [wave_loop_patterns.gd](../scripts/game_loop_waves_wave_loop_patterns.gd) (MultiMesh / async path), [wave_weighted_spawner.gd](../scripts/game_loop_waves_wave_weighted_spawner.gd) |
| **~10k bodies** | PhysicsServer / NavigationServer RIDs (no per-mob Node) | [wave_loop_patterns.gd](../scripts/game_loop_waves_wave_loop_patterns.gd) server-RID patterns; do **not** scale `wave_manager` node spawns |
...
1. Author a [wave_resource.gd](../scripts/game_loop_waves_wave_resource.gd) composition table.
2. Place [wave_spawner.gd](../scripts/game_loop_waves_wave_spawner.gd) Markers (or [wave_weighted_spawner.gd](../scripts/game_loop_waves_wave_weighted_spawner.gd) when variety weights matter).
3. Point [wave_manager.gd](../scripts/game_loop_waves_wave_manager.gd) `spawner` at that Marker; manager defers spawn and clears via `&"enemies"` group + signals.
...
Use `await` timers in [wave_manager.gd](../scripts/game_loop_waves_wave_manager.gd) — **MANDATORY read** before writing a custom timeline. Spawns use `call_deferred(&"add_child", …)`; clear via signals/groups (not `get_children()` scans).

### 2. Composition-Based Spawning
Define variety in [wave_resource.gd](../scripts/game_loop_waves_wave_resource.gd); place units with [wave_spawner.gd](../scripts/game_loop_waves_wave_spawner.gd) / [wave_weighted_spawner.gd](../scripts/game_loop_waves_wave_weighted_spawner.gd). Do not hardcode scene paths in the manager.
...
### [wave_spawner.gd](../scripts/game_loop_waves_wave_spawner.gd)
