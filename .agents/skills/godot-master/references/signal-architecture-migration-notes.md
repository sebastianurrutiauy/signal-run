...
- Prefer typed `signal_name.connect(callable)` over string connect.
- CanvasItem `hide` **signal** → `hidden` (method `hide()` unchanged).
- Tween signals → Tweener/`loop_finished` patterns.
- BaseButton: `pressed` property → `button_pressed`; signals `button_up`/`button_down`.
