...
> - Templates / base attributes → [base_stats_resource.gd](../scripts/rpg_stats_base_stats_resource.gd)
> - Runtime stack + reactive recalc → [stats_component_reactive.gd](../scripts/rpg_stats_stats_component_reactive.gd) + [stat_modifier_stacking.gd](../scripts/rpg_stats_stat_modifier_stacking.gd)
> - Buff/debuff data → [status_effect_data.gd](../scripts/rpg_stats_status_effect_data.gd) (`Type.ADDITIVE` / `MULTIPLICATIVE` / `OVERRIDE`)
> - Combat math → [damage_formula_handler.gd](../scripts/rpg_stats_damage_formula_handler.gd)
...
### [rpg_stat_resource.gd](../scripts/rpg_stats_rpg_stat_resource.gd)
Capped base stat Resource with setter clamps + `stat_changed` signal.

### [derived_stat_resource.gd](../scripts/rpg_stats_derived_stat_resource.gd)
Derived stat that recalculates when base stat dependencies change.

### [equipment_tooltip_helper.gd](../scripts/rpg_stats_equipment_tooltip_helper.gd)
...
- **NEVER calculate damage directly in the Character script** — Centralize in [damage_formula_handler.gd](../scripts/rpg_stats_damage_formula_handler.gd).
...
|-------|----------------|--------|
| **Resource template** | Designer-authored base attributes, curves, inheritance | [base_stats_resource.gd](../scripts/rpg_stats_base_stats_resource.gd), [exp_progression_resource.gd](../scripts/rpg_stats_exp_progression_resource.gd), [resource_stat_inheritance.gd](../scripts/rpg_stats_resource_stat_inheritance.gd) |
| **Runtime StatsComponent** | Duplicate/instance template, apply/remove modifiers, emit signals, JIT derived stats | **MANDATORY** [stats_component_reactive.gd](../scripts/rpg_stats_stats_component_reactive.gd) + [stat_modifier_stacking.gd](../scripts/rpg_stats_stat_modifier_stacking.gd) |
| **StatusEffectData** | Typed buff rows (`ADDITIVE` / `MULTIPLICATIVE` / `OVERRIDE`) | [status_effect_data.gd](../scripts/rpg_stats_status_effect_data.gd) |
| **DamageFormula (RefCounted)** | Pure combat math shared by Player/NPC | **MANDATORY** [damage_formula_handler.gd](../scripts/rpg_stats_damage_formula_handler.gd) |
| **Persistence** | Save progression; strip runtime buffs first | [persistent_character_stats.gd](../scripts/rpg_stats_persistent_character_stats.gd) |
| **UI sync** | Labels listen to signals — no polling | [dynamic_stat_label_sync.gd](../scripts/rpg_stats_dynamic_stat_label_sync.gd) |
...
Stacking / refresh / unique-vs-stackable behavior: **MANDATORY** [stat_modifier_stacking.gd](../scripts/rpg_stats_stat_modifier_stacking.gd). Apply through [stats_component_reactive.gd](../scripts/rpg_stats_stats_component_reactive.gd) so derived stats recalc once per change.

---

## Elite reminders (script-backed)

1. **Caps** — [rpg_stat_resource.gd](../scripts/rpg_stats_rpg_stat_resource.gd); clamp on setters; never allow overflow attributes.
2. **Dependency graphs** — [derived_stat_resource.gd](../scripts/rpg_stats_derived_stat_resource.gd); derived stats recalc from signals when bases change — never `_process`.
3. **Equipment** — Register/remove modifier IDs on equip/unequip via stacking API; tooltips via [equipment_tooltip_helper.gd](../scripts/rpg_stats_equipment_tooltip_helper.gd).

## Deep dive (load on demand)

Equipment hooks, damage formula, skill gates, elite cap/derived/tooltip patterns — [references/elite-stat-patterns.md](rpg-stats-elite-stat-patterns.md).
