...
| Mode | Use | Script |
|------|-----|--------|
| Hitscan | Pistols, ARs, snipers — instant feedback | [hitscan_weapon_query.gd](../scripts/genre_shooter_fps_hitscan_weapon_query.gd) |
| Projectile | Rockets, grenades — gravity arcs | [server_projectile_instance.gd](../scripts/genre_shooter_fps_server_projectile_instance.gd) (visual RID density) |
...
1. **Visual kick** — camera pivot ([procedural_recoil_handler.gd](../scripts/genre_shooter_fps_procedural_recoil_handler.gd))
2. **Pattern offset** — learnable spray ([recoil_system.gd](../scripts/genre_shooter_fps_recoil_system.gd) `recoil_pattern` array)
...
[aim_assist.gd](../scripts/genre_shooter_fps_aim_assist.gd) — friction slowdown (≈0.3) within `assist_angle` + subtle screen-space magnetism (≈0.1). Disable or narrow on PC mouse builds.
...
- **Impacts:** Decal pool + fade — [bullet_decal_spawner.gd](../scripts/genre_shooter_fps_bullet_decal_spawner.gd)
- **Camera:** FOV punch + short shake on fire
- **Viewmodel:** [weapon_bobbing_system.gd](../scripts/genre_shooter_fps_weapon_bobbing_system.gd) + `Input.get_last_mouse_velocity()` sway
