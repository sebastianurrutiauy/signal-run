...
> 1. [vn_rollback_manager.gd](../scripts/genre_visual_novel_vn_rollback_manager.gd) — history stack (flags/backgrounds/index)
> 2. [story_manager.gd](../scripts/genre_visual_novel_story_manager.gd) — flag-aware dialog orchestration
> 3. [dialogue_ui.gd](../scripts/genre_visual_novel_dialogue_ui.gd) — typewriter + choice UI
> 4. [visual_novel_patterns.gd](../scripts/genre_visual_novel_visual_novel_patterns.gd) — BBCode, choice filtering, sprite layering
...
- [dialogue_ui.gd](../scripts/genre_visual_novel_dialogue_ui.gd) - Presentation layer: typewriter tweens (`visible_ratio`) and choice-window generation.
- [vn_rollback_manager.gd](../scripts/genre_visual_novel_vn_rollback_manager.gd) - History stack for state rollback (flags/backgrounds/index).
- [visual_novel_patterns.gd](../scripts/genre_visual_novel_visual_novel_patterns.gd) - Reusable BBCode effects, choice filtering by flags, sprite layering.
...
Do not paste incomplete JSON StoryManager demos — implement from **MANDATORY** [story_manager.gd](../scripts/genre_visual_novel_story_manager.gd).

---

## Golden Path (order matters)

1. **Snapshot before mutate** — On every advance/choice, **MANDATORY** [vn_rollback_manager.gd](../scripts/genre_visual_novel_vn_rollback_manager.gd) pushes `{line_index, flags, background, music}` *before* flag writes.
2. **Typewriter + skip** — [dialogue_ui.gd](../scripts/genre_visual_novel_dialogue_ui.gd): Tween `visible_ratio` 0→1; on skip/advance input set `visible_ratio = 1.0` and kill the tween.
3. **Choice filter by flags** — Present only options whose `requires` StringName flags pass; apply choice → mutate flags → jump label ([visual_novel_patterns.gd](../scripts/genre_visual_novel_visual_novel_patterns.gd) + [story_manager.gd](../scripts/genre_visual_novel_story_manager.gd)).
