...
- **NEVER allow unfiltered user input in Chat Labels** — A user could type `[img]huge_image_path[/img]` or `[color=transparent]` to break your UI. **MANDATORY**: pipe every user-generated string through [rich_text_bbcode_sanitizer.gd](../scripts/ui_rich_text_rich_text_bbcode_sanitizer.gd) before assigning `text`.
...
| Pause / speed / event tags (`[pause]`, `[speed]`) | `visible_characters` + indexer | [rich_text_typewriter_controller.gd](../scripts/ui_rich_text_rich_text_typewriter_controller.gd) |
...
**MANDATORY**: [rich_text_bbcode_sanitizer.gd](../scripts/ui_rich_text_rich_text_bbcode_sanitizer.gd) on any chat, lobby, or player-typed path before `RichTextLabel.text = …`.

## Handle Link Clicks

Prefer [rich_text_meta_dispatch.gd](../scripts/ui_rich_text_rich_text_meta_dispatch.gd). Minimal hook:
...
**Pause/speed tags** — **MANDATORY** [rich_text_typewriter_controller.gd](../scripts/ui_rich_text_rich_text_typewriter_controller.gd) using `visible_characters` (not `visible_ratio`).
...
| Topic | Reference |
|-------|-----------|
| Tag catalog + 4.7 img units | [bbcode-tag-catalog.md](ui-rich-text-bbcode-tag-catalog.md) |
