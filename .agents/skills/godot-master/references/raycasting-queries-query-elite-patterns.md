...
See [mouse_pick_3d_query.gd](../scripts/raycasting_queries_mouse_pick_3d_query.gd).
...
Repeated `intersect_ray` with growing `exclude` RID list — [multiple_hit_piercing_ray.gd](../scripts/raycasting_queries_multiple_hit_piercing_ray.gd).

## Tunneling fast projectiles

Thin rays miss between frames — use `cast_motion` or ShapeCast stepping — [shapecast_ground_detection.gd](../scripts/raycasting_queries_shapecast_ground_detection.gd).
