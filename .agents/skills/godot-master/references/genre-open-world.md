...
- [global_state.gd](../scripts/genre_open_world_global_state.gd) - Chunk-keyed delta persistence (`set_entity_dead` pattern).
...
> **MANDATORY** for depth beyond decision trees and script catalog: [open-world-elite-implementations.md](genre-open-world-open-world-elite-implementations.md). **Do NOT Load** on first-pass wiring — use bundled `scripts/` first.
