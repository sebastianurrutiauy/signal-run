...
### [shader_warmup_loader.gd](../scripts/shaders_basics_shader_warmup_loader.gd)
...
| Topic | Reference |
|-------|-----------|
| 2D dissolve/wave/outline | [2d-effect-recipes.md](shaders-basics-2d-effect-recipes.md) |
| 3D toon + vignette | [3d-and-postfx-recipes.md](shaders-basics-3d-and-postfx-recipes.md) |
| Uniforms / built-ins | [uniforms-and-builtins.md](shaders-basics-uniforms-and-builtins.md) |
| Fog, compute, warmup | [expert-advanced-patterns.md](shaders-basics-expert-advanced-patterns.md) |
...
#### Prerequisites
- [godot-project-foundations](project-foundations.md) — Nodes, Resources, and project layout required before attaching ShaderMaterials and shipping `.gdshader` assets.
- [godot-resource-data-patterns](resource-data-patterns.md) — Sharing vs duplicating ShaderMaterial/Shader Resources so uniforms and instance parameters stay batch-friendly.

#### Complements
- [godot-3d-materials](3d-materials.md) — StandardMaterial3D/ORM first; graduate to spatial shaders for triplanar, dissolve, and instance-uniform effects.
- [godot-3d-lighting](3d-lighting.md) — How custom `ALBEDO`/`EMISSION`/`light()` output interacts with Forward+, GI, and fog volumes.
- [godot-particles](particles.md) — Particle process/draw materials and alpha pipelines that must match scissor/hash vs blend choices from this skill.
- [godot-2d-animation](2d-animation.md) — CanvasItem shader hooks for stylized 2D motion, outline, and dissolve on animated sprites.
- [godot-camera-systems](camera-systems.md) — Camera near/far and view/projection matrices that screen-space and depth-reconstruction shaders depend on.
- [godot-performance-optimization](performance-optimization.md) — Draw-call batching, MultiMesh, and GPU budgets that justify `instance uniform` and avoiding unique materials.
- [godot-debugging-profiling](debugging-profiling.md) — GPU/overdraw profilers and debug views to validate shader cost and depth/normal visualizers.

#### Downstream / consumers
- [godot-procedural-generation](procedural-generation.md) — Procedural meshes/terrain consume noise displacement, triplanar, and UV-less spatial patterns from this skill.
- [godot-3d-world-building](3d-world-building.md) — Large environment props apply foliage wind, grass flatten, and world-projection shaders at level scale.
- [godot-genre-open-world](genre-open-world.md) — Open-world foliage interaction, distance FX, and shared-material batching consume these shader templates.
