...
func _draw() -> void:
	...
	var blend := Engine.get_physics_interpolation_fraction() if model.state == "playing" else 1.0
	var render_player := model.previous_player.lerp(model.player, blend)
	...
	for index in model.enemies.size():
		var enemy := model.previous_enemies[index].lerp(model.enemies[index], blend)
		_shadow(enemy, 22)
	...
	draw_string(ThemeDB.fallback_font, Vector2(260, 51), "Cada señal acelera a los rastreadores. Evitá el contacto.", HORIZONTAL_ALIGNMENT_LEFT, -1, 13, Color("a8b4a8"))
	draw_rect(Rect2(754, 17, 174, 40), Color("28332c"))
	draw_string(ThemeDB.fallback_font, Vector2(770, 44), "%02d / %02d" % [model.collected, model.config.goal_spawns.size()], HORIZONTAL_ALIGNMENT_LEFT, -1, 21, Color("e0d4ac"))
	...
	draw_string(ThemeDB.fallback_font, Vector2(605, 520), "ESC   Pausa  /  R   Reiniciar al terminar", HORIZONTAL_ALIGNMENT_LEFT, -1, 13, Color("a5b0aa"))
	draw_string(ThemeDB.fallback_font, Vector2(48, 66), "TIEMPO %05.1f s   /   RASTREADORES %03d" % [model.elapsed, model.enemy_speed()], HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color("a8b4a8"))
	if model.state != &"playing":
		_draw_overlay()

func _draw_overlay() -> void:
	var title := "OPERACIÓN LISTA"
	var subtitle := "Recuperá %d señales. Cada una acelera a los rastreadores." % model.config.goal_spawns.size()
	var action := "ENTER / ESPACIO   Comenzar"
	if model.state == &"paused":
		title = "OPERACIÓN EN PAUSA"
		subtitle = "Tiempo y rastreadores detenidos."
		action = "ESC / ENTER   Continuar"
	elif model.state == &"won" or model.state == &"lost":
		title = "SEÑAL ASEGURADA" if model.state == &"won" else "TE DETECTARON"
		subtitle = "%d / %d señales  ·  %.1f s  ·  Victorias %d / %d" % [model.collected, model.config.goal_spawns.size(), model.elapsed, model.wins, model.attempts]
		action = "R / ENTER   Volver a intentarlo"
	draw_rect(Rect2(0, 0, 960, 540), Color(0.025, 0.04, 0.03, 0.80))
	draw_rect(Rect2(180, 175, 600, 190), Color("202b25"))
	draw_rect(Rect2(180, 175, 600, 190), Color("697567"), false, 1)
	var accent := Color("df9274") if model.state == &"lost" else Color("c9d5aa")
	draw_rect(Rect2(180, 175, 4, 190), accent)
	draw_string(ThemeDB.fallback_font, Vector2(212, 210), "INFORME DE OPERACIÓN", HORIZONTAL_ALIGNMENT_LEFT, -1, 11, Color("a2aba3"))
	draw_string(ThemeDB.fallback_font, Vector2(212, 250), title, HORIZONTAL_ALIGNMENT_LEFT, -1, 28, accent)
	draw_string(ThemeDB.fallback_font, Vector2(212, 286), subtitle, HORIZONTAL_ALIGNMENT_LEFT, -1, 15, Color("c1c9bf"))
	draw_string(ThemeDB.fallback_font, Vector2(212, 334), action, HORIZONTAL_ALIGNMENT_LEFT, -1, 17, Color("e0d4ac"))
